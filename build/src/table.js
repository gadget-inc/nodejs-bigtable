...
Please use the format 'prezzy' or '${instance.name}/tables/prezzy'.`);
    ...
    createReadStream(opts) {
        ...
        const ranges = table_1.TableUtils.getRanges(options);
        ...
        let lastRowKey = '';
        let rowsRead = 0;
        const userStream = new stream_1.PassThrough({
            objectMode: true,
            ...
            transform(row, _encoding, callback) {
                ...
                if (table_1.TableUtils.lessThanOrEqualTo(row.id, lastRowKey)) {
                    /*
        ...
        const rowStreamPipe = (rowStream, userStream) => {
            rowStream.pipe(userStream, { end: false });
            rowStream.on('end', originalEnd);
        };
        const rowStreamUnpipe = (rowStream, userStream) => {
            rowStream === null || rowStream === void 0 ? void 0 : rowStream.unpipe(userStream);
            rowStream === null || rowStream === void 0 ? void 0 : rowStream.removeListener('end', originalEnd);
        };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        userStream.end = (chunk, encoding, cb) => {
            ...
            if (lastRowKey) {
                ...
                for (let index = ranges.length - 1; index >= 0; index--) {
                    ...
                    const startKeyIsRead = !startValue ||
                        table_1.TableUtils.lessThanOrEqualTo(startValue, lastRowKey);
                    const endKeyIsNotRead = !endValue ||
                        endValue.length === 0 ||
                        table_1.TableUtils.lessThan(lastRowKey, endValue);
                    if (startKeyIsRead) {
                        if (endKeyIsNotRead) {
                ...
                rowKeys = rowKeys.filter(rowKey => table_1.TableUtils.greaterThan(rowKey, lastRowKey));
