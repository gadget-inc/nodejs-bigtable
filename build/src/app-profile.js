...
Please use the format 'my-app-profile' or '${instance.name}/appProfiles/my-app-profile'.`);
    ...
    static formatAppProfile_(options) {
        const appProfile = {};
        const errMessage = 'An app profile routing policy can only contain "any" for multi cluster routing, a `Cluster` for single routing, or a set of clusterIds as strings or `Clusters` for multi cluster routing.';
        if (options.routing) {
