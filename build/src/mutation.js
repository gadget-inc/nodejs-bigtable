...
class Mutation {
   ...
   * @example
    ...
    * @private
    */
    static encodeAddToCell(data) {
        const { family, qualifier } = Mutation.parseColumnName(data.column);
        let timestamp = data.timestamp || new Date();
        if (is.date(timestamp)) {
            timestamp = timestamp.getTime() * 1000;
        }
        return [
            {
                addToCell: {
                    familyName: family,
                    columnQualifier: Mutation.convertToBytes(qualifier),
                    timestamp: { rawTimestampMicros: timestamp },
                    input: Mutation.convertToBytes(data.input),
                }
            }
        ];
    }
